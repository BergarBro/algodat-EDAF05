...
public class Lab3 {
	...
	public static void main(String args[]) {
		...
		int sum = 0;
		int ant = 0;
		list[0].setDist(0);													// pick a random start node
		que.add(list[0]);													// put that node in the queue
		while (que.getSize() != 0 || ant == N - 1) {
			Vertex temp = que.getSmall();					// log n		// get the smallest node that is "near"
...
class Heap {
	...
	private void swap(int i1, int i2) {									// Swap to nodes in the heap
		Vertex temp = array[i1];										// and swap in "sort" accordingly 
